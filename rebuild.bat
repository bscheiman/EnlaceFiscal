@echo off
SET FILE=EnlaceFiscal
echo WScript.Echo DateDiff("s", "01/01/1970 00:00:00", Now()) > epoch.vbs
for /f "delims=" %%x in ('cscript /nologo epoch.vbs') do set epoch=%%x
del epoch.vbs
SET FULLVERSION=1.1.%epoch%

tools\nuget.exe restore EnlaceFiscal.sln -OutputDirectory %cd%\packages -NonInteractive
...
tools\nuget.exe pack EnlaceFiscal.nuspec -p Configuration=Release -Version %FULLVERSION%
